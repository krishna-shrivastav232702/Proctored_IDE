...
export const handleYjsSync = (teamId: string, userId: string, update: Uint8Array): void => {
    ...
    // Broadcast to other team members (excluding sender)
...
export const handleAwarenessSync = (teamId: string, userId: string, update: Uint8Array): void => {
    const room = rooms.get(teamId);
    if (!room) {
        console.error(`Yjs room not found for team ${teamId}`);
        return;
    ...
    // Broadcast awareness update to all team members
    emitToTeam(teamId, "awareness:update", { 
        update: Array.from(update), 
        userId,
        timestamp: Date.now(),
    });
...
export const handleAwarenessDisconnect = (teamId: string, userId: string): void => {
    const room = rooms.get(teamId);
    if (!room) return;

    // Broadcast awareness removal to team
    emitToTeam(teamId, "awareness:remove", { 
        userId,
        timestamp: Date.now(),
    });
}
